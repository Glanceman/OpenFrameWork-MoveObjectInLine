...
void ofApp::setup(){
	ofSetFrameRate(60);
	gui.setup();
	gui.add(lowThreshold.setup("low threshold for canny", 50, 0, 100));
	gui.add(lineVoteThreshold.setup("line vote threshold", 10, 0, 200));
	gui.add(finishBtn.setup("apply"));
	finishBtn.addListener(this,&ofApp::finishBtnEvent);
	minLength = 1;
	maxGap = 0;
	//load image
	//img.load("line2.jpg"); //image 1
	//img.load("line3.jpg"); //image 2
	img.load("line4.jpg"); //image 3
...
void ofApp::update(){
	...
	if (bfinish) {
		...
		//sort line based on bottom right to top-left direction
